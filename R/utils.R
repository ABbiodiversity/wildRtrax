...
#' Translation of point count report to ARU for internal use in data wrangling functions.
#'
#' @description Translation of point count sensor main report to field names used in ARU report
#'
#' @param data WildTrax main report from the `wt_download_report()` function for the point count sensor.
#'
#' @keywords internal
#'
#' @import dplyr
#'
.pc_to_aru <- function(data){

  data2 <- data %>%
    rename(task_id=survey_id,
           recording_date_time = survey_date) %>%
    mutate(task_method = "PC",
...
#' Translation of point count field names back from ARU field names for internal use in data wrangling functions.
#'
#' @description Translation of point count sensor main report back to original field names
#'
#' @param data WildTrax main report from the `wt_download_report()` function for the point count sensor.
#'
#' @keywords internal
...
#' @import dplyr
#'
.aru_to_pc <- function(data){

  data2 <- data %>%
    rename(survey_id=task_id,
           survey_date = recording_date_time) %>%
    dplyr::select(-task_method, -observer_id)
