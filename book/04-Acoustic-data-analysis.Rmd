...
raw_basic_data <- dir_ls(path = "/users/alexandremacphail/desktop/fn", regexp = "*basic_summary.csv") %>%
  map_df(~read_csv(., col_types = cols(recording_date  = col_character())))

data_logistic_wtsp <- raw_basic_data %>% 
  filter(!species_code %in% abiotic_codes,
         !grepl('^U',species_code)) %>%
  mutate(present = case_when(species_code == "WTSP" ~ 1, TRUE ~ 0)) %>%
  select(location, latitude, longitude, recording_date, species_code, present) %>%
  distinct() %>%
  mutate(hour = hour(recording_date)) %>%
  pivot_wider(names_from = species_code, values_from = present, values_fill = 0)
         
         
# create a logistic regression model
mylogit <- glm(species_presence ~ location + recording_date_time + forest_type, data = data_logistic_wtsp, family = binomial)
