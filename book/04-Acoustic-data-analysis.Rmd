...
model_acc <- mean(predicted.classes == test$SOSP)

model_acc
```

The model therefore has a probability of `r percent(model_acc)` of detecting presence of Song Sparrows near water taking into account julian date.
...
x <- vegan::specaccum(dj[,3:228], method = "random")
boxplot(x)
...
diversity(dj[,3:228], index="shannon") %>%
  as_tibble() %>%
  bind_cols(., dj %>% select(location)) %>%
