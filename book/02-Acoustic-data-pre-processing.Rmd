...
# LUs 
LUs <- read_sf("/users/alexandremacphail/wildRtrax/data/OSR_LU_fxd/OSR_LUs_fxd.shp") %>%
  st_make_valid() %>%
  mutate(year = case_when(OBJECTID %in% c(2,4,6,8) ~ "2021",
                          OBJECTID %in% c(1, 13, 15, 21) ~ "2022",
                          OBJECTID %in% c(9, 22, 14, 16) ~ "2023",
                          TRUE ~ NA_character_))
...
```{r, comment="", warning=F, message=F, prompt=T}
ggplot(LUs) +
  geom_sf(data = bcrs %>% filter(BCRNAME == "BOREAL TAIGA PLAINS"), mapping = aes(), fill = NA) +
  geom_sf(mapping = aes(fill = year)) +
  geom_sf_text(aes(label = ifelse(is.na(year), "", OBJECTID)), colour = "black") +
  theme_bw() +
  scale_fill_viridis_d()
...
Once your acoustic recordings are organized and files chosen for processing, the next step is to upload them to WildTrax for processing.
