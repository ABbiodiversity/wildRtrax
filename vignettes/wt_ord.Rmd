...
cls <- read_csv("/users/alexandremacphail/desktop/jjj.csv") %>%
...
data <-
  dir_ls(path = "/users/alexandremacphail/desktop/ab", regexp = "*tag_details_report.csv") %>%
...
fn <- dir_ls(path = "/users/alexandremacphail/desktop/ab", regexp = "*recording_birdnet.csv") %>%
...
res <- wt_ord(input = data, min_obs = 8, confidence = 0.67)
