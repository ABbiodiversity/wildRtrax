...
Explore species and data within WildTrax's [Data Discover](https://discover.wildtrax.ca/) by employing the `wt_dd_summary()` function. Access a restricted portion of data, even without public user privileges. Utilize `wt_auth()` to uncover data pertinent to your account or those publicly available on WildTrax.

```{r, echo=T, include=T, eval=F, warning=F, message = FALSE}
discover <- wt_dd_summary(sensor = "ARU", species = "White-throated Sparrow", boundary = NULL)
```
```{r, echo=F, include=F, eval=T, warning=F, message = FALSE}
discover <- wt_dd_summary(sensor = "ARU", species = c("White-throated Sparrow"), boundary = NULL)
```
```{r, eval=T}
head(discover)
