...
audio_files <-
  ...
  future_map_dbl(., .f = ~ file_size(.), .progress = TRUE, .options = furrr_options(seed = TRUE)) %>%
  enframe() %>%
  mutate(
    size = round(value / 10e5, digits = 2),   # Convert size to megabytes
    filename = basename(name),
    filetype = as_factor(file_ext(filename)),
    compressed = case_when(filetype %in% c('wac', 'mp3', 'flac', '.w4v', 'wav') ~ TRUE, TRUE ~ FALSE)) %>%
...
* **Date time**
  * Contains a POSIX.ct formattable date time
