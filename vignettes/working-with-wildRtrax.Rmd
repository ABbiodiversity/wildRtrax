...
audio_files <-
  ...
  purrr::future_map_dbl(., .f = ~ file_size(.), .progress = TRUE, .options = furrr_options(seed = TRUE)) %>%
  tibble::enframe() %>%
  dplyr::mutate(
    size = round(value / 10e5, digits = 2),   # Convert size to megabytes
    filename = basename(name),
    filetype = as_factor(tools::file_ext(filename)),
    compressed = dplyr::case_when(filetype %in% c('wac', 'mp3', 'flac', '.w4v', 'wav') ~ TRUE, TRUE ~ FALSE)) %>%
...
wac <- bioacoustics::read_wac(file_wac)

flac <- bioacoustics::wav2flac(file_flac, reverse = TRUE) #If needing to convert to a flac use reverse = FALSE
...
audio_files <- audio_files %>%
  dplyr::filter(filename %in% c('ABMI-0320-NE_0+1_20180529_054500.wav','ABMI-0320-NE_0+1_20180528_054600.wav'))
...
pics <- list.files("",".png", recursive = T, full.names = T)
...
audio_files <- 
  fs::dir_ls(path = path,
    ...
    dplyr::mutate(
      ...
      julian = lubridate::yday(recording_date_time),
      year = lubridate::year(recording_date_time),
      gps_enabled = dplyr::case_when(grepl('$',file_name)) ~ TRUE #Find gps enable recordings using the '$' character
    ) %>%
...
audio_files <- wt_audio_scanner(path = , file_type = )
...
## **Advanced audio data manipulation**
