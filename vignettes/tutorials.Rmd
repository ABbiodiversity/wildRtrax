...
birdnet_report <- data[[1]] %>%
  filter(is_species_allowed_in_project == 't',
         confidence > 50) %>%
...
merged_data %>%
  filter(observer == "BirdNET" & presence == 1) %>%
  select(location, recording_date_time, species_code) %>%
  anti_join(
    merged_data %>%
      filter(observer != "BirdNET") %>%
      select(location, recording_date_time, species_code, presence),
