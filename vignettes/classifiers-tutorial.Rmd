...
Once a task has been transcribed, the BirdNET results are available with 24 hours via [WildTrax](http://www.wildtrax.ca) or the `wildrtrax` R package using the `wt_download_report()` function. Before then the report will simply show NOT GENERATED entries for each recording.

Use `wt_download_report(reports = c('main','birdnet'))` to download the BirdNET report and the main report for further analysis. We'll use the ABMI Ecosystem Health dataset from 2022 as an example dataset. The output will be a list of tibbles in alphabetical order.
...
A precision at our chosen score threshold of approximately `round(eval$precision,2)` means that less than half of these detections are likely still false positives, we should probably visually verify to remove those false positives. We can use the `wt_format_detections()` function to format our detections object into the format required to upload to WildTrax for verification.
...
There are `unique(new$species_code)` potential new species detections in our dataset. We can use the `wt_format_detections()` function to format our output for upload to WildTrax and verification. 
