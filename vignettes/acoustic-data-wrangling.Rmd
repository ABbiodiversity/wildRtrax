...
my_tidy_data <- wt_tidy_species(my_report, remove = c("abiotic","mammal"), zerofill=F, sensor = 'ARU')

# Difference in rows
round((nrow(my_tidy_data)/nrow(my_report)),2)
```

So about 5% of detections were mammals. Next, convert TMTT (too many to tag) counts to numeric:
...
```{r, eval = F, echo = T, warning = F, message = F}
my_wide_data <- wt_make_wide(data = my_tmtt_data, sound = "all", sensor = 'ARU')
...
```{r, eval = F, echo = T, warning = F, message = F}
my_offset_data <- wt_qpad_offsets(data = my_wide_data, species = "all", version = 3, together = TRUE, sensor = 'ARU')
...
```{r, eval = F, echo = T, warning = F, message = F}
dat.occu <- wt_format_occupancy(my_report, species="WTSP", siteCovs=NULL, sensor = 'ARU')
